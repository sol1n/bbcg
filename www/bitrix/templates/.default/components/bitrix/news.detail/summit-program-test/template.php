...
<nav class="subnav program-table-subnav">
    <div class="wrapper">
        ...
        <?
            $program_file = $arResult['PROPERTIES']['PROGRAM_FOR_DOWNLOAD']['VALUE'];
            $en_program_file = $arResult['PROPERTIES']['EN_PROGRAM_FOR_DOWNLOAD']['VALUE'];
        ?>
        <? if((!empty($program_file)) && ($arParams['LANG'] != 'en')):?>
        ...
        <? elseif((!empty($en_program_file)) && ($arParams['LANG'] == 'en')):?>
            <? $download_link = CFile::GetPath($arResult['PROPERTIES']['EN_PROGRAM_FOR_DOWNLOAD']['VALUE']); ?>
            <li class="program-table-pagination-item active program-download-button">
                <a href="<?=$download_link?>" target="_blank" download="<?=$arResult['NAME']?>">
                    <?=Loc::GetMessage('DOWNLOAD PROGRAM', [], $arParams['LANG'])?>
                </a>
