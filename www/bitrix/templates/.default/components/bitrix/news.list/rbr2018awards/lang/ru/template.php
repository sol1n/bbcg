<?
	$MESS["REGISTRATION"] = "Заявка на участие";
	$MESS["SELECT_NOMINATION"] = "Выберите номинацию";
	$MESS["COMPANY"] = "Компания";
	$MESS["CONTACTS"] = "Контактная информация";
	$MESS["WHY_DESERVES"] = "Объясните, почему ваша компания заслуживает этой награды";
	$MESS["FILE"] = "Файл";
	$MESS["CHOOSE_FILE"] = "Выберите файл";

	$MESS["DO_REGISTER"] = "Зарегистрироваться";
	$MESS["CONTACT_PERSON"] = "Контактное лицо Ксения Черенкова";
	$MESS["WE_WILL_CONTACT_YOU"] = "Нажимая кнопку «Зарегистрироваться», я принимаю условия <a href=\"/eula/\" target=\"_blank\">Пользовательского соглашения</a> и даю согласие на обработку персональных данных.";
	$MESS["ALTERNAME_REGISTRATION"] = "Альтернативные способы регистрации";
	$MESS["DOWNLOAD_AND_SEND_REQUEST"] = "Скачать заявку и отправить на указанную почту";
?>
