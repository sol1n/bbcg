<div class="horizontal-banner banner">
    <a class="banner-close"></a>
    <div class="wrapper">
        <div class="row">
            <div class="col-md-2 col-sm-2 banner-left">
                <a href="/exclusive/makeretailgreatagain/" target="_blank">
                    <img src="/assets/images/banner-element.png"/>
                </a>
            </div>
            <div class="col-md-8 col-sm-8 banner-center">
                <img src="/assets/images/banner-central.png"/>
            </div>
            <div class="col-md-2 col-sm-2 banner-right">
                <a class="banner-button button button-light-burgundy" href="/exclusive/makeretailgreatagain/" target="_blank">ЧИТАТЬ ЖУРНАЛ</a>
            </div>
        </div>
    </div>
</div>
