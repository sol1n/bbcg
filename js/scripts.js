...
//$("#sticky_item").stick_in_parent({offset_top: 90});


if ($(window).width() > 1023){// фиксировать блок контактов в разделе партнеры у декстопных устройствах
    ...
    function Ascroll() {
      ...
      var Ra = a.getBoundingClientRect(),
          R = Math.round(Ra.top + b.getBoundingClientRect().height - 50 - document.querySelector('.main-footer').getBoundingClientRect().top + 60);  // селектор блока, при достижении верхнего края которого нужно открепить прилипающий элемент;  Math.round() только для IE; если ноль заменить на число, то блок будет прилипать до того, как нижний край элемента дойдёт до футера
      if ((Ra.top - P) <= 0) {
        if ((Ra.top - P) <= R) {
          b.className = 'stop';
          b.style.top = - R +'px';
